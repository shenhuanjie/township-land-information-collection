...
public class Collection_Market_Supply_House_Rent extends Fragment implements OnClickListener, OnTouchListener {
	...
	private EditText edit_nczlc;// 拟出租楼层
	private Spinner edit_nczhyt;// 拟出租用途
	private EditText edit_nczmj;// 拟出租建筑面积
	private EditText edit_nczsj;// 拟出租时间
	private EditText edit_nzq;// 拟租期(年)
	private EditText edit_nzj;// 拟年租金
	...
	private String value_nczlc;// 拟出租楼层
	private String value_nczhyt;// 拟出租用途
	private String value_nczmj;// 拟出租建筑面积
	private String value_nczsj;// 拟出租时间
	private String value_nzq;// 拟租期(年)
	private String value_nzj;// 拟年租金
	...
	private ArrayList<String> list_nczyt = new ArrayList<String>();// 拟出租用途
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View view = (View) inflater.inflate(R.layout.collection_market_supply_house_rent_frame, container, false);
		initView(view);
		initEditText(view);
		initSpinnerDict();
	...
	private void initView(View view) {
		...
		textView_Tips.setText("房地出租供应信息");
	}
	...
	private void initEditText(View view) {
		...
		edit_czf = (EditText) view.findViewById(R.id.edit_czf);// 出租方
		...
		edit_nczlc = (EditText) view.findViewById(R.id.edit_nczlc);// 拟出租楼层
		edit_nczhyt = (Spinner) view.findViewById(R.id.edit_nczhyt);// 拟出租用途
		edit_nczmj = (EditText) view.findViewById(R.id.edit_nczmj);// 拟出租建筑面积
		edit_nczsj = (EditText) view.findViewById(R.id.edit_nczsj);// 拟出租时间
		edit_nzq = (EditText) view.findViewById(R.id.edit_nzq);// 拟租期(年)
		edit_nzj = (EditText) view.findViewById(R.id.edit_nzj);// 拟年租金
	...
	private void initSpinnerDict() {
		...
		list_nczyt = typeUtil.initList("ytlx");// 拟出租用途
	...
	private void initOnItemSelectedListener() {
		...
		edit_nczhyt.setOnItemSelectedListener(new Spinner.OnItemSelectedListener() {
			@Override
			public void onItemSelected(AdapterView<?> arg0, View arg1, int index, long arg3) {
				value_nczhyt = list_nczyt.get(index);
				index_nczyt = index;
			}
	...
	public void onClick(View arg0) {
		...
		case R.id.fragment_collection_ibtn_save:
			...
			if (edit_x.getText().toString().equals("") || edit_y.getText().toString().equals("")) {
				...
				locationInfo.setCollectionType(CollectType.Collection_Market_Supply_House_Rent);
				// 类型ID
				locationInfo.setCollectionMainType(CollectType.Supply);
				locationInfo.setCollectionTableName(CollectType.Collection_Market_Supply_House_Rent_Tablename);// 表名
				// 存储的实体类型
	...
	private void setCollectionInfo() {
		...
		value_czf = edit_czf.getText().toString();// 出租方
		...
		value_nczlc = edit_nczlc.getText().toString();// 拟出租楼层
		value_nczmj = edit_nczmj.getText().toString();// 拟出租建筑面积
		value_nczsj = edit_nczsj.getText().toString();// 拟出租时间
		value_nzq = edit_nzq.getText().toString();// 拟租期(年)
		value_nzj = edit_nzj.getText().toString();// 拟年租金
		...
		/** 位置信息.变量 **/
		value_x = edit_x.getText().toString();// X坐标
		value_y = edit_y.getText().toString();// Y坐标
	...
	private int saveCollectionInfo() {
		int result = 0;
		setCollectionInfo();
		ModelCollectionMarketSupplyHouseRent model = new ModelCollectionMarketSupplyHouseRent();
		/** 基本信息 **/
		...
		model.setNCZLC(value_nczlc);// 拟出租楼层
		model.setNCZHYT(value_nczhyt);// 拟出租用途
		model.setNCZMJ(value_nczmj);// 拟出租建筑面积
		model.setNCZSJ(value_nczsj);// 拟出租时间
		model.setNZQ(value_nzq);// 拟租期(年)
		model.setNZJ(value_nzj);// 拟年租金
		...
		/** 位置信息 **/
		model.setX(value_x);// X坐标
		model.setY(value_y);// Y坐标
		...
		model.setIndex_zzhx(index_zxcd);
		model.setIndex_zzhx(index_zzhx);
		model.setIndex_ptss(index_ptss);
