 ...
 * 
 */
public class EditCollectionMarketSupplyTransferActivity extends ActionBarActivity implements OnClickListener, OnTouchListener {
	...
	private void initView() {
		...
		text_tips.setText("土地使用权转让供应信息");// TODO 修改提示信息
	}
	...
	private void initModel() {
		...
		if (model != null) {
			...
			setEditText(R.id.edit_tdzl, "TDZL", model.getTDZL());// 土地坐落
			setEditText(R.id.edit_qsxz, "QSXZ", model.getQSXZ());// 权属性质
			setEditText(R.id.edit_zrf, "ZRF", model.getZRF());// 转让方
			setEditText(R.id.edit_nzrsj, "NZRSJ", model.getNZRSJ());// 拟转让时间
			setEditText(R.id.edit_nzrfs, "NZRFS", model.getNZRFS());// 拟转让方式
			setEditText(R.id.edit_nzrmj, "NZRMJ", model.getNZRMJ());// 拟土地转让面积
			setEditText(R.id.edit_zrqyt, "ZRQYT", model.getZRQYT());// 转让前用途
			setEditText(R.id.edit_nzrhyt, "NZRHYT", model.getNZRHYT());// 拟转让后用途
			setEditText(R.id.edit_tdsynq, "TDSYNQ", model.getTDSYNQ());// 土地剩余使用年期
			setEditText(R.id.edit_zrdj, "ZRDJ", model.getZRDJ());// 拟土地转让单价
			setEditText(R.id.edit_zrzj, "ZRZJ", model.getZRZJ());// 拟土地转让总价
