 ...
 * 
 */
public class EditCollectionMarketDemandSellActivity extends ActionBarActivity implements OnClickListener, OnTouchListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.collection_market_demand_sell_frame_detail);
		/** 初始化控件 **/
		initView();
		initEditText();
		/** 初始化变量 **/
	...
	private void initView() {
		...
		text_tips.setText("土地使用权出售需求信息");// TODO 修改提示信息
	}
	...
	private void initModel() {
		ModelCollectionMarketDemandSell model = DataSupport.find(ModelCollectionMarketDemandSell.class, locationInfo.getCollectionId());
		if (model != null) {
			/** 基本信息 **/
			...
			/** 土地使用权出让需求信息 **/
			setEditText(R.id.edit_xqf, "XQF", model.getXQF());// 需求方
			setEditText(R.id.edit_xqfztxz, "XQFZTXZ", model.getXQFZTXZ());// 需求方主体性质
			setEditText(R.id.edit_ntdzl, "NTDZL", model.getNTDZL());// 拟土地坐落
			setEditText(R.id.edit_nqsxz, "NQSXZ", model.getNQSXZ());// 拟权属性质
			setEditText(R.id.edit_xqcrsj, "XQCRSJ", model.getXQCRSJ());// 需求出让时间
			setEditText(R.id.edit_xqcrfs, "XQCRFS", model.getXQCRFS());// 需求出让方式
			setEditText(R.id.edit_nxqcrmj, "NXQCRMJ", model.getNXQCRMJ());// 拟需求出让面积
			setEditText(R.id.edit_crqyt, "CRQYT", model.getCRQYT());// 出让前用途
			setEditText(R.id.edit_ncrhyt, "NCRHYT", model.getNCRHYT());// 拟出让后用途
			setEditText(R.id.edit_xqcrnq, "XQCRNQ", model.getXQCRNQ());// 需求出让年期
			setEditText(R.id.edit_ghrjl, "GHRJL", model.getGHRJL());// 规划宗地容积率
			setEditText(R.id.edit_xqxztdtj, "XQXZTDTJ", model.getXQXZTDTJ());// 需求现状土地条件
			setEditText(R.id.edit_xqcrdj, "XQCRDJ", model.getXQCRDJ());// 需求单位面积土地出让价
			setEditText(R.id.edit_xqcrzj, "XQCRZJ", model.getXQCRZJ());// 需求土地出让总价
			setEditText(R.id.edit_ntzqd, "NTZQD", model.getNTZQD());// 拟投资强度
			setEditText(R.id.edit_nkftzze, "NKFTZZE", model.getNKFTZZE());// 拟开发投资总额
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		getMenuInflater().inflate(R.menu.edit_collection_market_demand_sell, menu);
		return true;
	}
	...
	public void onClick(View arg0) { // TODO
		...
		case R.id.fragment_collection_ibtn_save:
			...
			if (edit_x.getText().toString().equals("") || edit_y.getText().toString().equals("")) {
				...
				locationInfo.setCollectionType(CollectType.Collection_Market_Demand_Sell);// TODO
																							// 存储的实体类型
				/*
