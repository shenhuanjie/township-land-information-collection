 ...
 * 
 */
public class EditCollectionMarketDemandShareHolderActivity extends ActionBarActivity implements OnClickListener, OnTouchListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.collection_market_demand_share_holder_frame_detail);
		/** 初始化控件 **/
		initView();
		initEditText();
		/** 初始化变量 **/
	...
	private void initView() {
		...
		text_tips.setText("土地联营入股需求信息");// TODO 修改提示信息
	}
	...
	private void initModel() {
		...
		if (model != null) {
			...
			/** 土地联营入股需求信息 **/
			setEditText(R.id.edit_xqczf, "XQCZF", model.getXQCZF());// 需求出资方
			setEditText(R.id.edit_xqczfztxz, "XQCZFZTXZ", model.getXQCZFZTXZ());// 需求出资方主体性质
			...
			setEditText(R.id.edit_xqlysj, "XQLYSJ", model.getXQLYSJ());// 需求联营时间
			setEditText(R.id.edit_xqlyq, "XQLYQ", model.getXQLYQ());// 需求联营期
			setEditText(R.id.edit_xqcdmj, "XQCDMJ", model.getXQCDMJ());// 需求出地方投入土地面积
			setEditText(R.id.edit_xqczze, "XQCZZE", model.getXQCZZE());// 需求出资方投入资金总额
			setEditText(R.id.edit_xqcdffcbl, "XQCDFFCBL", model.getXQCDFFCBL());// 需求出地方利润分成比例
			setEditText(R.id.edit_ntdncsr, "NTDNCSR", model.getNTDNCSR());// 拟单位面积土地年纯收入
			setEditText(R.id.edit_ncdfncsr, "NCDFNCSR", model.getNCDFNCSR());// 拟出地方土地年纯收入
			setEditText(R.id.edit_lyqyt, "LYQYT", model.getLYQYT());// 联营前用途
			setEditText(R.id.edit_nlyhyt, "NLYHYT", model.getNLYHYT());// 拟联营后用途
	...
	public void onClick(View arg0) { // TODO
		...
		case R.id.fragment_collection_ibtn_save:
			...
			if (edit_x.getText().toString().equals("") || edit_y.getText().toString().equals("")) {
				...
				locationInfo.setCollectionType(CollectType.Collection_Market_Demand_ShareHolder);// TODO
				// 存储的实体类型
				/*
