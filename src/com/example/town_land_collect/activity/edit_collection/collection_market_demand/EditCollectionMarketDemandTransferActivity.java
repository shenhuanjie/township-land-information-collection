 ...
 * 
 */
public class EditCollectionMarketDemandTransferActivity extends ActionBarActivity implements OnClickListener, OnTouchListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.collection_market_demand_transfer_frame_detail);
		/** 初始化控件 **/
		initView();
		initEditText();
		/** 初始化变量 **/
	...
	private void initView() {
		...
		text_tips.setText("土地使用权转入需求信息");// TODO 修改提示信息
	}
	...
	private void initModel() {
		ModelCollectionMarketDemandTransfer model = DataSupport.find(ModelCollectionMarketDemandTransfer.class,
		...
		if (model != null) {
			...
			setEditText(R.id.edit_xqzrsj, "XQZRSJ", model.getXQZRSJ());// 需求转让时间
			setEditText(R.id.edit_xqzrfs, "XQZRFS", model.getXQZRFS());// 需求转让方式
			setEditText(R.id.edit_nxqzrmj, "NXQZRMJ", model.getNXQZRMJ());// 拟需求土地转让面积
			setEditText(R.id.edit_zrqyt, "ZRQYT", model.getZRQYT());// 转让前用途
			setEditText(R.id.edit_zrqyt, "ZRQYT", model.getNZRHYT());// 拟转让后用途
			setEditText(R.id.edit_tdsynq, "TDSYNQ", model.getTDSYNQ());// 土地剩余使用年期
			setEditText(R.id.edit_ghrjl, "GHRJL", model.getGHRJL());// 规划宗地容积率
			setEditText(R.id.edit_xqzrdj, "XQZRDJ", model.getXQZRDJ());// 需求土地转让单价
			setEditText(R.id.edit_xqzrzj, "XQZRZJ", model.getXQZRZJ());// 需求土地转让总价
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		getMenuInflater().inflate(R.menu.edit_collection_market_demand_transfer, menu);
		return true;
	}
	...
	public void onClick(View arg0) { // TODO
		...
		case R.id.fragment_collection_ibtn_save:
			...
			if (edit_x.getText().toString().equals("") || edit_y.getText().toString().equals("")) {
				...
				locationInfo.setCollectionType(CollectType.Collection_Market_Demand_Transfer);// TODO
				// 存储的实体类型
				/*
