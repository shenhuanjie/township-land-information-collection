 ...
 * 
 */
public class EditCollectionMarketBusinessSellActivity extends ActionBarActivity implements OnClickListener, OnTouchListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.collection_market_business_sell_frame_detail);
		/** 初始化控件 **/
		initView();
		initEditText();
		/** 初始化变量 **/
	...
	private void initView() {
		...
		text_tips.setText("土地使用权出让交易信息");// TODO 修改提示信息
		edit_crsj = (EditText) findViewById(R.id.edit_crsj);
		edit_crsj.setOnTouchListener(this);
		edit_crnq = (EditText) findViewById(R.id.edit_crnq);
	...
	private void initModel() {
		ModelCollectionMarketBusinessSell model = DataSupport.find(ModelCollectionMarketBusinessSell.class, locationInfo.getCollectionId());
		if (model != null) {
			/** 基本信息 **/
			setEditText(R.id.edit_the_name, "edit_the_name", model.getTHENAME());// 记录名称
			setEditText(R.id.edit_the_code, "edit_the_code", model.getTHECODE());// 记录代码
			setEditText(R.id.edit_tdsyz, "edit_tdsyz", model.getTDSYZ());// 土地所有者
			setEditText(R.id.edit_tdzsh, "edit_tdzsh", model.getTDZSH());// 土地证书号
			setEditText(R.id.edit_tdzl, "edit_tdzl", model.getTDZL());// 土地坐落
			setSpinnerSelection(R.id.edit_qsxz, "edit_qsxz", model.getINDEX_QSXZ());// 权属性质
			setEditText(R.id.edit_crsj, "edit_crsj", model.getCRSJ());// 出让时间
			setEditText(R.id.edit_srf, "edit_srf", model.getSRF());// 受让方
			setSpinnerSelection(R.id.edit_crfs, "edit_crfs", model.getINDEX_CRFS());// 出让方式
			setSpinnerSelection(R.id.edit_sfzkf, "edit_sfzkf", model.getINDEX_SFZKF());// 是否再开发
			setSpinnerSelection(R.id.edit_sflyd, "edit_sflyd", model.getINDEX_SFLYD());// 是否留用地
			setEditText(R.id.edit_ghtdmj, "edit_ghtdmj", model.getGHTDMJ());// 规划红线范围土地面积
			setEditText(R.id.edit_dztdmj, "edit_dztdmj", model.getDZTDMJ());// 代征土地面积
			setEditText(R.id.edit_sjcrmj, "edit_sjcrmj", model.getSJCRMJ());// 实际出让面积
			setSpinnerSelection(R.id.edit_crqyt, "edit_crqyt", model.getINDEX_CRQYT());// 出让前用途
			setSpinnerSelection(R.id.edit_crhyt, "edit_crhyt", model.getINDEX_CRHYT());// 出让后用途
			setEditText(R.id.edit_crnq, "edit_crnq", model.getCRNQ());// 出让年期
			setEditText(R.id.edit_crdj, "edit_crdj", model.getCRDJ());// 单位面积土地出让价
			setEditText(R.id.edit_crzj, "edit_crzj", model.getCRZJ());// 土地出让总价
			setEditText(R.id.edit_rjl, "edit_rjl", model.getRJL());// 宗地容积率
			setSpinnerSelection(R.id.edit_hxwkfsp, "edit_hxwkfsp", model.getINDEX_HXWKFSP());// 红线外开发水平
			setSpinnerSelection(R.id.edit_hxnkfsp, "edit_hxnkfsp", model.getINDEX_HXNKFSP());// 红线内开发水平
			setSpinnerSelection(R.id.edit_tdjb, "edit_tdjb", model.getINDEX_TDJB());// 所在土地级别
			setEditText(R.id.edit_djqd, "edit_djqd", model.getDJQD());// 所在地价区段
			setEditText(R.id.edit_xzqdm, "edit_xzqdm", model.getXZQDM());// 行政区代码
			setEditText(R.id.location_info_lon, "location_info_lon", model.getX()); // x坐标
			setEditText(R.id.location_info_lat, "location_info_lat", model.getY());// Y坐标
			setEditText(R.id.location_info_zbxt, "location_info_zbxt", model.getZBXT());// 坐标系统
			setEditText(R.id.location_info_remark, "location_info_remark", model.getREMARK());// 备注
		}
	}
				...
				public void afterTextChanged(Editable s) {
					if (s != null && !s.equals("")) {
						if (MIN_MARK != -1 && MAX_MARK != -1) {
							...
							if (markVal > MAX_MARK) {
								Toast.makeText(getApplicationContext(), "不能超过100", Toast.LENGTH_SHORT).show();
